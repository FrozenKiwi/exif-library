(function (global, factory) {
  ...
  (global = global || self, factory(global.exifLib = {}));
          ...
          this.convertValue = (field) => {
              ...
              if (t == 1) {
                  ...
                  if (length > 4) {
                      ...
                      data = unpack(this.endianMark + repeatString("B", length), this.tiftag.slice(pointer, pointer + length));
                  }
                  else {
                      data = unpack(this.endianMark + repeatString("B", length), value.slice(0, length));
                  }
              }
              else if (t == 2) {
                  // ASCII
                  ...
                  if (length > 2) {
                      pointer = unpack(this.endianMark + "L", value)[0];
                      data = unpack(this.endianMark + repeatString("H", length), this.tiftag.slice(pointer, pointer + length * 2));
                  }
                  else {
                      data = unpack(this.endianMark + repeatString("H", length), value.slice(0, length * 2));
                  }
              }
              else if (t == 4) {
                  // LONG
                  if (length > 1) {
                      pointer = unpack(this.endianMark + "L", value)[0];
                      data = unpack(this.endianMark + repeatString("L", length), this.tiftag.slice(pointer, pointer + length * 4));
                  }
                  else {
                      data = unpack(this.endianMark + repeatString("L", length), value);
                  }
              }
              else if (t == 5) {
                  // RATIONAL
  ...
  const load = (bytes) => {
      ...
      const zerothIfdPointer = unpack(exifReader.endianMark + "L", exifReader.tiftag.slice(IFD_POINTER_BEGIN, IFD_POINTER_BEGIN + IFD_POINTER_LENGTH))[0];
      ...
      if (zerothIfd !== null && TagNumbers.ImageIFD.ExifTag in zerothIfd) {
          ...
          exifIfd = exifReader.getIfd(exifIfdPointer, "Exif");
      ...
      if (zerothIfd !== null && TagNumbers.ImageIFD.GPSTag in zerothIfd) {
          ...
          gpsIfd = exifReader.getIfd(gpsIfdPointer, "GPS");
          exifObj["GPS"] = gpsIfd;
      }
      if (exifIfd !== null && TagNumbers.ExifIFD.InteroperabilityTag in exifIfd) {
          ...
          interopIfd = exifReader.getIfd(interopIfdPointer, "Interop");
      ...
      if (firstIfdPointerBytes != "\x00\x00\x00\x00") {
          const firstIfdPointer = unpack(exifReader.endianMark + "L", firstIfdPointerBytes)[0];
          ...
          if (firstIfd !== null &&
              ...
              const thumbnailEnd = firstIfd[TagNumbers.ImageIFD.JPEGInterchangeFormat] +
                  firstIfd[TagNumbers.ImageIFD.JPEGInterchangeFormatLength];
              thumbnail = exifReader.tiftag.slice(firstIfd[TagNumbers.ImageIFD.JPEGInterchangeFormat], thumbnailEnd);
              exifObj["thumbnail"] = thumbnail;
          }
      }
      return exifObj;
  };
  const getExifBytes = (bytes) => {
      let exifBytes;
      if (typeof bytes == "string") {
          if (bytes.slice(0, 2) == "\xff\xd8") {
              exifBytes = bytes;
          }
          else if (bytes.slice(0, 23) == "data:image/jpeg;base64," ||
              bytes.slice(0, 22) == "data:image/jpg;base64,") {
              exifBytes = atob(bytes.split(",")[1]);
          }
  ...
  const packByte = (array) => {
      return pack(">" + repeatString("B", array.length), array);
  };
  const packShort = (array) => {
      return pack(">" + repeatString("H", array.length), array);
  };
  const packLong = (array) => {
      return pack(">" + repeatString("L", array.length), array);
  };
  const toByte = (rawValue, offset) => {
      ...
      if (length <= 4) {
          tagBinary.value =
              packByte(rawValue) + repeatString("\x00", 4 - length);
      }
      else {
  ...
  const toAscii = (rawValue, offset) => {
      ...
      if (length > 4) {
          ...
          tagBinary.value = newValue + repeatString("\x00", 4 - length);
      }
      tagBinary.lengthBinary = pack(">L", [length]);
      return tagBinary;
  };
  const toShort = (rawValue, offset) => {
      if (typeof rawValue == "number") {
          rawValue = [rawValue];
      }
      else if (Array.isArray(rawValue) && typeof rawValue[0] === "number") ;
      ...
      if (length <= 2) {
          tagBinary.value =
              packShort(rawValue) + repeatString("\x00\x00", 2 - length);
      }
      else {
          tagBinary.value = pack(">L", [offset]);
  ...
  const toRational = (rawValue, offset) => {
      let value;
  ...
  const toUndefined = (rawValue, offset) => {
      if (typeof rawValue !== "string") {
          const t = Array.isArray(rawValue) ? "Array" : typeof rawValue;
          throw new ValueConvertError(`Value must be "string". Got "${t}".`);
      }
      ...
      if (length > 4) {
          ...
          tagBinary.value = rawValue + repeatString("\x00", 4 - length);
      }
      tagBinary.lengthBinary = pack(">L", [length]);
      return tagBinary;
  };
  const toSRational = (rawValue, offset) => {
      let value;
      if (Array.isArray(rawValue) &&
          typeof rawValue[0] === "number" &&
          rawValue.length === 2) {
  ...
  const valueToBytes = (rawValue, valueType, offset) => {
      let tagBinary;
      if (valueType == Types.Byte) {
          tagBinary = toByte(rawValue, offset);
      }
      else if (valueType == Types.Ascii) {
          tagBinary = toAscii(rawValue, offset);
      }
      else if (valueType == Types.Short) {
          tagBinary = toShort(rawValue, offset);
      }
      else if (valueType == Types.Long) {
          tagBinary = toLong(rawValue, offset);
      }
      else if (valueType == Types.Rational) {
          tagBinary = toRational(rawValue, offset);
      }
      else if (valueType == Types.Undefined) {
          tagBinary = toUndefined(rawValue, offset);
      }
      else if (valueType == Types.SLong) {
          throw new Error("Not implemented for SLong value");
      }
      else if (valueType == Types.SRational) {
          tagBinary = toSRational(rawValue, offset);
      }
      else {
  ...
  const dump = (originalExifObj) => {
      const TIFF_HEADER_LENGTH = 8;
      const exifObj = copyObject(originalExifObj);
      ...
      if (("Exif" in exifObj && Object.keys(exifObj["Exif"]).length) ||
          ...
          else if (Object.keys(exifIfd).indexOf(TagNumbers.ExifIFD.InteroperabilityTag.toString()) > -1) {
      ...
      else if (Object.keys(zerothIfd).indexOf(TagNumbers.ImageIFD.GPSTag.toString()) > -1) {
      ...
      if (existFirstIfd) {
          ...
          const thumbnailPointerBytes = "\x02\x01\x00\x04\x00\x00\x00\x01" +
              pack(">L", [thumbnailPointer]);
          const thumbnailLengthBytes = "\x02\x02\x00\x04\x00\x00\x00\x01" +
      ...
      const zerothIfdBytes = zerothIfdSet[0] +
          exifPointer +
          gpsPointer +
          firstIfdPointer +
          zerothIfdSet[1];
  ...
  const insert = (exifBytes, imageBytes) => {
      let base64Encoded = false;
      if (exifBytes.slice(0, 6) != "\x45\x78\x69\x66\x00\x00") {
          throw new Error("Given data is not exif.");
      }
      if (imageBytes.slice(0, 2) == "\xff\xd8") ;
      else if (imageBytes.slice(0, 23) == "data:image/jpeg;base64," ||
          imageBytes.slice(0, 22) == "data:image/jpg;base64,") {
          imageBytes = atob(imageBytes.split(",")[1]);
          base64Encoded = true;
      }
      else {
          throw new Error("Given data is not jpeg.");
      }
      const app1Segment = "\xff\xe1" + pack(">H", [exifBytes.length + 2]) + exifBytes;
      const segments = splitIntoSegments(imageBytes);
      let newBytes = mergeSegments(segments, app1Segment);
      if (base64Encoded) {
          newBytes = "data:image/jpeg;base64," + btoa(newBytes);
      }
      return newBytes;
  };

  const remove = (imageBytes) => {
...
//# sourceMappingURL=exif-library.js.map
